...
**1-1. 화살표 모드** — 판서 중 `A`
`A`를 한 번 누르면 그냥 드래그만 해도 화살표가 그려집니다. 파워포인트처럼 **드래그를 끝낸 곳에 화살촉**이 생깁니다.
다시 `A`를 누르면 원래 펜으로 돌아옵니다. 켜져 있는 동안 오른쪽 아래에 `Arrow` 표시가 뜹니다.
원래 ZoomIt 방식(`⇧⌃` 드래그, 시작한 곳에 화살촉)도 그대로 쓸 수 있고, 화살촉은 크고 속이 찬 삼각형으로 바꿨습니다.
