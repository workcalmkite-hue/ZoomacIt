...
- `Esc` 또는 오른쪽 클릭으로 취소.
- **`Ctrl+2`로 그려 둔 판서 위에서 눌러도 됩니다.** 판서가 있는 그대로 찍히고,
  캡처가 끝나면 그리던 화면으로 그대로 돌아옵니다 (지워지지 않습니다).
  확대(정지) 화면 위에서 눌러도 마찬가지로 확대된 그대로 찍힙니다.
  라이브 확대(`Ctrl+8`)만 예외로, 그 창은 원래 캡처에서 제외됩니다(무한 거울 방지).

### 브레이크 타이머 위젯

- 10:00 부터 시작하고 **멈춘 상태로 뜹니다** — 재생(▶)을 눌러야 갑니다.
- 원을 드래그해 이동, **휠로 크기 조절**(70~220), 위치·크기는 다음 실행 때 복원됩니다.
- **`−` `▶` `＋` `✕` 네 버튼은 사라지지 않습니다.** 타이머가 도는 동안에는 옅게 두고,
  마우스를 올리면 또렷해집니다 — 멈추거나 시간을 고치려고 위젯을 더듬을 일이 없습니다.
- 멈춰 있거나 시간이 다 된 뒤에는 마우스를 올리지 않아도 계속 또렷합니다.
